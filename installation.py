...
def install_on_macOS():
    try:
        ...
        if (platform.python_version() != "3.11.11"):
            ...
            os.system("uv run --python pypy@3.11.11 -- python --version")
...
def install_on_linux():
    try:
        ...
        # Update the config file
        configFilePath = Path.home() / ".config" / "Claude" / "claude_desktop_config.json"
        update_config_file(configFilePath)
    except FileNotFoundError as e:
        print(f"{e}")
        sys.exit(1)
...
def main():
    os_type = platform.system()
    if os_type == "Linux":
        print("Running installation for Linux...")
        install_on_linux()
    elif os_type == "Darwin":
